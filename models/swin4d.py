...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, corr_size=(16, 16, 16, 16),
        ...
        self.query_pos_embed = nn.Parameter(torch.zeros(1, embed_dim//2, *corr_size[:-2], 1, 1))# 生成查询图像尺度的位置编码，[B,32,8,8,1,1]
        self.supp_pos_embed = nn.Parameter(torch.zeros(1, embed_dim//2, 1, 1, *corr_size[-2:]))# 生成支撑图像尺度的位置编码，[B,32,1,1,8,8]
    ...
    def forward(self, x):
        query_pos_embed = repeat(self.query_pos_embed, 'B L H_q W_q a b -> B L H_q W_q (H_s a) (W_s b)', H_s=self.corr_size[2], W_s=self.corr_size[3])# 将[B,32,8,8,1,1]扩充成[B,32,8,8,8,8]，适配滑动窗口
        ...
        pos_embed = torch.cat((query_pos_embed, supp_pos_embed), dim=1)# 拼接扩充后的查询位置编码与支撑位置编码，(B, 64, 8, 8, 8, 8)
        x = x + rearrange(pos_embed, 'B L H_q W_q H_s W_s -> B (H_q W_q H_s W_s) L')# 位置编码转化为vit输入格式并相加x(B, 8*8*8*8, 64)
        x = self.pos_drop(x)
        return self.layer(x)

# 转化corr形状为BNC，经过SwimTransformer后再输出
