...
class CustomMultiheadAttention(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        #
        self.diff_linear = nn.Linear(embed_dim, embed_dim)
        # 输出线性层
    ...
    def forward(self, query, key, value):
        ...
        attn_scores = torch.matmul(Q, K.transpose(-2, -1))  # [num_heads, batch_size, seq_len, seq_len]

        # 缩放点积相似度，避免数值过大
        #attn_scores = attn_scores / seq_len  # 缩放，防止数值过大
        ...
        # 计算注意力输出
        attn_output = torch.matmul(attn_weights, V)  # [num_heads, batch_size, seq_len, head_dim]
        attn_output = attn_output.permute(1, 2, 0, 3).contiguous()  # [batch_size, seq_len, num_heads, head_dim]
        attn_output = attn_output.view(batch_size, seq_len, self.num_heads * self.head_dim)  # [batch_size, seq_len, embed_dim]
        output = self.output_linear(attn_output - value)  # [batch_size, seq_len, embed_dim]
...
class CrossAttn(nn.Module):
    def __init__(self, feat_dims=[256,512,1024]):
        super().__init__()
        '''
        self.attn1 = nn.MultiheadAttention(feat_dims[0], 8)
        self.attn2 = nn.MultiheadAttention(feat_dims[1], 8)
        self.attn3 = nn.MultiheadAttention(feat_dims[2], 8)
        '''
        self.attn1 = CustomMultiheadAttention(feat_dims[0], 2)
        self.attn2 = CustomMultiheadAttention(feat_dims[1], 2)
        self.attn3 = CustomMultiheadAttention(feat_dims[2], 2)
       
        # Layer Normalization
        self.norm1 = nn.ModuleList([nn.LayerNorm(dim) for dim in feat_dims])
        self.norm2 = nn.ModuleList([nn.LayerNorm(dim) for dim in feat_dims])
        # Feedforward Layers
    ...
    def forward(self, query_list, match_list):
        attn_out = []
        for layer_idx in range(len(query_list)):
            query = query_list[layer_idx]
            match = match_list[layer_idx]
            b,_,h,w = query.shape
            query = query.permute(0, 2, 3, 1).view(b, h * w, -1)
            match = match.permute(0, 2, 3, 1).view(b, h * w, -1)
            if layer_idx == 0:
                out = self.attn1(query, match, match)
                attn_output = self.norm1[layer_idx](out + query)
                # Feedforward Network
                ffn_output = self.ffn[layer_idx](attn_output)
                # Residual Connection and Layer Normalization
                ffn_output = self.norm2[layer_idx](attn_output + ffn_output)
                diff = ffn_output - query
                attn_out.append(diff)
            elif layer_idx == 1:
                out = self.attn2(query, match, match)
                attn_output = self.norm1[layer_idx](out + query)
                # Feedforward Network
                ffn_output = self.ffn[layer_idx](attn_output)
                # Residual Connection and Layer Normalization
                ...
                out = self.attn3(query, match, match)
                attn_output = self.norm1[layer_idx](out + query)
                # Feedforward Network
                ffn_output = self.ffn[layer_idx](attn_output)
                # Residual Connection and Layer Normalization
...
class FussionDecoder(nn.Module):
    def __init__(self, feature_dim):
        super().__init__()
        self.feature_dim = feature_dim
        self.l2_to_l1 = UpsampleConv(feature_dim[1], feature_dim[0], scale_factor=2)
        self.l3_to_l2 = UpsampleConv(feature_dim[2], feature_dim[1], scale_factor=2)
        
        self.mask_mlp = nn.Sequential(
            nn.Linear(256*2, 128*2),  # 第一层，降维到1024
            nn.ReLU(inplace=True),
            nn.Linear(128*2, 1),  # 第二层，降维到1
            nn.Sigmoid()
        ) 

    def forward(self, ref_list):
        layer3_x_to_2 = self.l3_to_l2(ref_list[2])
        layer2_x_to_1 = self.l2_to_l1(ref_list[1] + layer3_x_to_2)
        out = ref_list[0] + layer2_x_to_1

        #out = self.upsample(out)
        B, C, H, W = out.shape
        
        # 调整形状为 [B * H * W, C]
        out = out.permute(0, 2, 3, 1).reshape(-1, C)  # [B, H, W, C] -> [B * H * W, C]
        out = self.mask_mlp(out)  # [B * H * W, 1]
        out = out.reshape(B, H, W, 1)  # [B * H * W, 1] -> [B, H, W, 1]
        out = out.permute(0, 3, 1, 2)  # [B, H, W, 1] -> [B, 1, H, W]
...
class DownsampleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, scale_factor=0.5):
        super().__init__()
        assert scale_factor == 0.5, "仅支持分辨率减半"
        self.conv = nn.Conv2d(
            in_channels, 
            out_channels, 
            kernel_size=3, 
            stride=2,  # 通过stride=2实现下采样
            padding=1
        )
...
class FussionDecoder(nn.Module): 
    def __init__(self, feature_dim):
        ...
        # 定义三个转换模块
        #self.l1_to_l2 = DownsampleConv(feature_dim[0], feature_dim[1])  # 512->1024, 64x64->32x32
        #self.l3_to_l2 = UpsampleConv(feature_dim[2], feature_dim[1])     # 2048->1024, 16x16->32x32
        
    def forward(self, ref_list):
        """
        输入特征列表格式：
        - ref_list[0]: [4, 512, 64, 64]
        - ref_list[1]: [4, 1024, 32, 32]
        """
        # 将layer1下采样到目标尺度
        #layer1_to_l2 = self.l1_to_l2(ref_list[0])  # [4,1024,32,32]
        
        #out = ref_list[1] + layer1_to_l2
        out = ref_list[1]
        
        return out  # 输出形状 [4,1024,32,32]
    
class head(nn.Module):
    def __init__(self):
        super().__init__()
        self.mask_mlp = nn.Sequential(
            nn.Linear(2048, 512),  # 第一层，降维到1024
            nn.ReLU(inplace=True),
            nn.Linear(512, 1),  # 第二层，降维到1
            nn.Sigmoid()
        ) 
    ...
    def forward(self, diff):
        ...
        out = diff.permute(0, 2, 3, 1).reshape(-1, C)  # [B, H, W, C] -> [B * H * W, C]
        out = self.mask_mlp(out)  # [B * H * W, 1]
        out = out.reshape(B, H, W, 1)  # [B * H * W, 1] -> [B, H, W, 1]
        out = out.permute(0, 3, 1, 2)  # [B, H, W, 1] -> [B, 1, H, W]
        ...
        #scores = out.flatten(start_dim=1).max(dim=1)[0]
